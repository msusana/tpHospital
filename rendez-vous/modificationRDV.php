...
$selectPatient =$pdo->prepare('SELECT * FROM patients');
$selectPatient ->execute();

$selectRDV =$pdo->prepare('SELECT * FROM appointments WHERE id=?');
$selectRDV ->execute([
    $_GET["id"]
    ]);

$dato=$selectRDV->fetch();
$id=$dato['id'];

$changedate= explode(" ", $dato['dateHour']);
$date = $changedate[0];
$Hour = $changedate[1]; 
...
<body>
    <h1>Formulaire rendez-vous patient</h1>
    <form action='/Exercice-PDO-2/recuperation_donnees/uprdv.php ' method='post'>
    <input type="hidden" name="IdRDV" value="<?php echo $id?>">
        ...
        <div>
            <label for='Hour' class='form-label'>Date</label>
            <input type='time' class='form-control' name='Hour' value="<?=$Hour?>" required >
        </div>
        
        <div>
        <select name='idPatients'>
        <?php foreach($selectPatient->fetchAll()as $patient){ 
            if($patient['id']===$dato['idPatients']){
                $selected= "selected";
            }
            else{
                $selected="";
